...
#include "OpenWorldFabService.h"

#if !WITH_OPENWORLD_FAB

// This engine install has no Fab plugin (Engine/Plugins/Fab), so OpenWorldMCP.Build.cs compiled OpenWorldFabService
...
static FString FabUnavailableJson()
{
	return TEXT("{\"success\":false,\"error_code\":\"UNSUPPORTED\",")
	       TEXT("\"error\":\"OpenWorldFabService is unavailable: this engine install does not include the Fab plugin ")
	       TEXT("(Engine/Plugins/Fab), so OpenWorldMCP was compiled without Fab support. ")
	       TEXT("Use an engine install that ships the Fab plugin and rebuild OpenWorldMCP to enable it.\"}");
}

FString UOpenWorldFabService::AuthStatus(float) { return FabUnavailableJson(); }
FString UOpenWorldFabService::ListLibrary(const FString&, const FString&, const FString&, int32, int32, bool) { return FabUnavailableJson(); }
FString UOpenWorldFabService::GetAsset(const FString&) { return FabUnavailableJson(); }
FString UOpenWorldFabService::SearchFreeCatalog(const FString&, const FString&, const FString&, int32, const FString&) { return FabUnavailableJson(); }
FString UOpenWorldFabService::ImportAsset(const FString&, const FString&, const FString&, const FString&) { return FabUnavailableJson(); }
FString UOpenWorldFabService::ImportFreeAsset(const FString&, const FString&, const FString&, const FString&, bool) { return FabUnavailableJson(); }
FString UOpenWorldFabService::ImportStatus(const FString&) { return FabUnavailableJson(); }

#else // WITH_OPENWORLD_FAB
...
DEFINE_LOG_CATEGORY_STATIC(LogOpenWorldFab, Log, All);
...
// ---------------------------------------------------------------------------
// JSON response helpers (same contract as the other OpenWorldMCP services)
// ---------------------------------------------------------------------------
...
static FString BaseUrl()
{
	...
	return OpenWorldMCP::Fab::ProdBaseUrl();
}
...
{
	if (!FOpenWorldFabAuth::IsSupported())
	{
		OutErr = ErrJson(TEXT("UNSUPPORTED"), TEXT("The EOS SDK is not available in this editor build, so Fab auth is unavailable."));
		return false;
	}
	FString Err;
	if (!FOpenWorldFabAuth::EnsureLoggedIn(TimeoutSeconds, Err))
	{
		const FString State = FOpenWorldFabAuth::GetStateString();
...
FString UOpenWorldFabService::AuthStatus(float TimeoutSeconds)
{
	if (!FOpenWorldFabAuth::IsSupported())
	{
	...
	FString Err;
	const bool bLoggedIn = FOpenWorldFabAuth::EnsureLoggedIn(FMath::Max(1.0f, TimeoutSeconds), Err);
	const FString State = FOpenWorldFabAuth::GetStateString();
	...
	if (bLoggedIn)
	{
		Obj->SetStringField(TEXT("epic_account_id"), FOpenWorldFabAuth::GetEpicAccountId());
...
FString UOpenWorldFabService::ListLibrary(const FString& NameFilter, const FString& TypeFilter, const FString& EngineVersion,
...
FString UOpenWorldFabService::GetAsset(const FString& AssetId)
{
	if (AssetId.IsEmpty())
	{
...
FString UOpenWorldFabService::SearchFreeCatalog(const FString& Query, const FString& SellerFilter,
	                                    const FString& FormatFilter, int32 Limit, const FString& Cursor)
{
	...
	FString Error;
	int32 HttpCode = 0;
	if (!FOpenWorldFabCatalog::SearchFree(BaseUrl(), Query, SellerFilter, FormatFilter, Limit, Cursor,
...
FString UOpenWorldFabService::ImportAsset(const FString& AssetId, const FString& EngineVersion, const FString& Quality, const FString& Format)
{
	if (AssetId.IsEmpty())
	{
		return ErrJson(TEXT("BAD_ARGUMENT"), TEXT("AssetId is required."));
	...
	if (TSharedPtr<FFabImportProgress> Prev = FOpenWorldFabImport::Get(AssetId))
	{
	...
	int32 HttpCode = 0;
	FString MErr;
	if (!FOpenWorldFabManifest::Fetch(BaseUrl(), ArtifactId, A->AssetNamespace, A->AssetId, TEXT("Windows"),
	...
	FString SErr;
	if (!FOpenWorldFabImport::Start(AssetId, A->Title, bIsPlugin, Info, SErr))
	{
...
FString UOpenWorldFabService::ImportFreeAsset(const FString& ListingId, const FString& Quality, const FString& Format,
	                                  const FString& LicenseSlug, bool AcceptEula)
{
	...
	if (TSharedPtr<FFabImportProgress> Previous = FOpenWorldFabImport::Get(ListingId))
	{
	...
	FString Error;
	int32 HttpCode = 0;
	if (!FOpenWorldFabCatalog::ResolveFreeDownload(BaseUrl(), ListingId, LicenseSlug, Quality, Format,
	...
	if (!FOpenWorldFabImport::StartDirect(ListingId, Download.Title, Download.DownloadUrl, Destination, Error))
	{
...
FString UOpenWorldFabService::ImportStatus(const FString& AssetId)
{
	TSharedPtr<FFabImportProgress> P = FOpenWorldFabImport::Get(AssetId);
	if (!P)
	{
