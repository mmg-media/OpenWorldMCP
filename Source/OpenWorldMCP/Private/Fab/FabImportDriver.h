...
/**
 * Import driver: reuses the engine Fab plugin's FAB_API BuildPatch download machinery
 * (FFabDownloadRequest), then implements the post-download step (asset-registry scan + /Game path
 * reporting) itself instead of relying on engine-internal Private import workflows. Async — Start()
 * kicks the download and returns; poll via Get(). Supports BuildPatch pack/plugin assets plus public
 * free glTF/GLB ZIP downloads (including Quixel/Megascans) through automated Interchange import.
 */
class FOpenWorldFabImport
{
public:
	/**
