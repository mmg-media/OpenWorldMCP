...
bool FOpenWorldFabCatalog::SearchFree(const FString& BaseUrl, const FString& Query, const FString& SellerFilter,
	                              ...
	                              FString& OutNextCursor, int32& OutHttpCode, FString& OutError)
{
	...
	for (int32 Page = 0; Page < 5 && OutListings.Num() < SafeLimit; ++Page)
	{
		...
		const FString Url = OpenWorldMCP::Fab::CatalogSearchUrl(BaseUrl, Query, SellerFilter, PageCursor);
		FString Body;
...
bool FOpenWorldFabCatalog::ResolveFreeDownload(const FString& BaseUrl, const FString& ListingId,
	                                       ...
	                                       int32& OutHttpCode, FString& OutError)
{
	...
	FString Body;
	if (!SyncGet(OpenWorldMCP::Fab::CatalogListingUrl(BaseUrl, ListingId), TimeoutSeconds, OutHttpCode, Body)
	...
	if (!SyncGet(OpenWorldMCP::Fab::CatalogFormatsUrl(BaseUrl, ListingId), TimeoutSeconds, OutHttpCode, Body)
		|| OutHttpCode < 200 || OutHttpCode >= 300)
	{
	...
	const FString DownloadInfoUrl = OpenWorldMCP::Fab::CatalogDownloadInfoUrl(
