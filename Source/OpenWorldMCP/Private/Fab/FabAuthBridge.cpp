...
namespace
{
	...
	{
		UObject* Asset = FSoftObjectPath(OpenWorldMCP::Fab::EosCredsAssetPath()).TryLoad();
		if (!Asset)
		{
...
bool FOpenWorldFabAuth::IsSupported()
{
#if WITH_EOS_SDK
	IEOSSDKManager* SDK = IEOSSDKManager::Get();
...
bool FOpenWorldFabAuth::EnsureLoggedIn(double TimeoutSeconds, FString& OutError)
{
#if WITH_EOS_SDK
...
bool FOpenWorldFabAuth::IsLoggedIn()
{
#if WITH_EOS_SDK
...
FString FOpenWorldFabAuth::GetAccessToken()
{
#if WITH_EOS_SDK
	if (!IsLoggedIn())
	{
...
FString FOpenWorldFabAuth::GetEpicAccountId()
{
#if WITH_EOS_SDK
...
FString FOpenWorldFabAuth::GetStateString()
{
#if WITH_EOS_SDK
	switch (GState)
	{
...
FString FOpenWorldFabAuth::GetLastError()
{
	return GLastError;
}
