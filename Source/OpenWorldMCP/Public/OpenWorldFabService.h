...
#include "OpenWorldFabService.generated.h"

/**
 * Fab asset import service. It can discover/import the signed-in Epic account's owned
 * library, or search the public zero-price catalog and directly import eligible glTF/GLB
 * listings (including Quixel Megascans) without adding them to the account library.
 *
 * Thin orchestration over Unreal 5.8's built-in Fab plugin: it reuses the Epic login the
 * editor/launcher already holds (via its own EOS session), calls fab.com's owned-library
 * REST feed, and (for import) drives the engine Fab plugin's downloader. These fab.com
 * endpoints are unofficial/reverse-engineered and may change without notice.
 *
...
class OPENWORLDMCP_API UOpenWorldFabService : public UToolsetDefinition
{
	GENERATED_BODY()
...
public:
	 ...
	 * Silently reuses the editor/launcher Epic login; if none is available, returns actionable
	 * guidance (open the Fab window / launch from the Epic Games Launcher) rather than failing hard.
	 * @param TimeoutSeconds How long to wait for the (async) login to resolve before reporting pending. Default 15.
	 */
	UFUNCTION(BlueprintCallable, meta = (AICallable), Category = "OpenWorld|Fab")
	static FString AuthStatus(float TimeoutSeconds = 15.0f);

	/**
	 * List the owned Fab library as a COMPACT projection (id, title, type, source, distribution
	 * method, a `compatible` flag for the current engine, thumbnail). Fetched once and cached
	 * in-process; pass Refresh=true to re-fetch. Filters are applied locally.
